...
const PageHome = () => {
  ...
  const sendGetDetailListing = (id) => {
    ...
    return fetch(`http://google.hongkong.styuan990102.top:5005/listings/${id}`, body);
  }
  ...
  const getAllListing = async (email) => {
    ...
    if (response.status === 200) {
      ...
      if (token) {
        const allBookingsResponse = await fetch('http://google.hongkong.styuan990102.top:5005/bookings', body);
